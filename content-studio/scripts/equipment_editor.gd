...
var _visual_texture_cache: Dictionary = {}
...
func _derive_visual_key() -> String:
	if not _equippable.button_pressed:
		return "None"
	var value := _normalize_visual_key(_display_name.text.strip_edges())
...
func _update_paper_doll_preview() -> void:
	if _paper_doll_layers.is_empty():
		return
	for slot_name in _paper_doll_layers:
	...
	var selected_slot := _selected_metadata(_slot) if _equippable.button_pressed else ""
	var selected_key := _derive_visual_key()
	if selected_slot in PAPER_DOLL_VISIBLE_SLOTS and selected_key != "None":
	...
	for slot_name in PAPER_DOLL_LAYER_ORDER:
		...
		if asset_key.is_empty():
			continue
		var texture := _load_visual_texture(slot_name, asset_key, frame, direction)
		if texture == null:
			continue
		var layer := _paper_doll_layers[slot_name] as TextureRect
	...
	elif selected_slot == "ring":
		_paper_doll_status.text = "Ring is gameplay equipment but has no visible paper-doll layer in the current client."
	elif loaded_selected:
		_paper_doll_status.text = "%s • frame %d • visual key %s" % [direction, frame, selected_key]
	else:
		_paper_doll_status.text = "No player-layer PNG matched %s in slot %s for %s frame %d." % [selected_key, selected_slot, direction, frame]


func _load_visual_texture(slot_name: String, asset_key: String, frame: int, direction: String) -> Texture2D:
	for fallback_frame in _visual_frame_fallbacks(frame, direction):
		var file_path := _find_visual_file(slot_name, asset_key, int(fallback_frame), direction)
		if file_path.is_empty():
			continue
...
func _find_visual_file(slot_name: String, asset_key: String, frame: int, direction: String) -> String:
	...
	if _visual_file_cache.has(cache_key):
		return str(_visual_file_cache[cache_key])
	var directory_path := _game_client_assets_root.path_join("actors").path_join("player").path_join(slot_name)
	...
	if FileAccess.file_exists(canonical_path):
		_visual_file_cache[cache_key] = canonical_path
		return canonical_path
	var directory := DirAccess.open(directory_path)
	if directory == null:
		_visual_file_cache[cache_key] = ""
		return ""
	...
	while true:
		...
		if _legacy_visual_key(file_name.trim_suffix(suffix)) == asset_key:
			...
			_visual_file_cache[cache_key] = matched_path
			return matched_path
	directory.list_dir_end()
	_visual_file_cache[cache_key] = ""
	return ""
...
func _legacy_visual_key(base_name: String) -> String:
	...
	return _normalize_visual_key(prefix_expression.sub(base_name, "", false))


func _normalize_visual_key(value: String) -> String:
...
func _visual_frame_fallbacks(frame: int, direction: String) -> Array:
	var values: Array = [frame]
	if direction == "N" and not values.has(4):
		values.append(4)
	if not values.has(DEFAULT_PREVIEW_FRAME):
		values.append(DEFAULT_PREVIEW_FRAME)
	return values


func _paper_doll_z_index(slot_name: String, direction: String) -> int:
	match slot_name:
		"right_hand":
