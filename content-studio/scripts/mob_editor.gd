...
func _build_ui() -> void:
	...
	for option in [
		...
		_preview_direction.add_item(str(option.get("display_name", "")))
		_preview_direction.set_item_metadata(_preview_direction.item_count - 1, str(option.get("id", "")))
	_preview_direction.item_selected.connect(_on_preview_pose_changed.unbind(1))
	pose_row.add_child(_preview_direction)
	pose_row.add_child(_label("Frame"))
	...
	pose_row.add_child(_preview_frame)
	preview_content.add_child(pose_row)
...
func _update_visual_preview() -> void:
	...
	if _selected_metadata(_visual_mode) == "composite_rig":
		...
		var preview_state: Dictionary = _composite_preview.update_composite(
			...
			_selected_metadata(_preview_direction),
